...
This is the UART transmitter, the whole program. `D[1]` is the bit period, which the host sets. `pull` loads the outgoing shift register from the host FIFO and `out` shifts bits from it onto a pin. `at +1` means "one bit period after the cursor". Here that is the same as one period after the previous edge because every instruction in the chain has it, except `at +1 nop`, which moves the cursor without any edge. An instruction with no prefix at all is `at +0`, and `D[0]` is always zero, so its effect lands on the cursor and leaves it there:
